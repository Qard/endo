...
static inline endo::Environment* Env(Isolate* isolate) {
  return static_cast<endo::Environment*>(isolate->GetData(0));
}
static inline endo::Environment* Env(Local<Context> context) {
  return Env(context->GetIsolate());
}

static inline endo::Environment* Env(const FunctionCallbackInfo<Value>& info) {
  return Env(info.GetIsolate());
}

static inline endo::Environment* Env(const PropertyCallbackInfo<Value>& info) {
  return Env(info.GetIsolate());
}
